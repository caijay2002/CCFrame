...
#region << 默认语言 >>
/*----------------------------------------------------------------
// 文件名称：DefaultLanguage
// 创 建 者：蔡程健
// 创建时间：22/5/23 14:59:34
// 文件版本：
// ===============================================================
// 功能描述：
//		
...
namespace CCFrame.Language
{
    public class DefaultLanguage
    {
        ...
        public virtual string AuthorizationFailed =>
            ("系统授权失败，需要使用激活码授权，谢谢支持。设备激活数");
        //("系统授权失败，需要使用激活码授权，谢谢支持。设备激活数：" + HslCommunication.Authorization.iahsduiwikaskfhishfdi.ToString());
        ...
        public virtual string DLTAddressMatchFailed =>
            "地址格式配对失败，请检查是否是少于12个字，且都是0-9或A的数字组成的地址";
        ...
        public virtual string InsufficientPrivileges =>
            "当前的方法接口或类，只对商业授权用户开放，权限不足，感谢支持。如果需要商业授权，联系QQ200962190，微信：13516702732，Email:hsl200909@163.com";

        public virtual string IpAddressError =>
            "Ip地址输入异常，格式不正确";

        public virtual string KeyenceNanoE0
        {
            get
            {
                string[] textArray1 = new string[] { "1. 指定的软元件编号、存储体编号、单元编号、地址超出范围。", Environment.NewLine, "2. 指定了程序不用的定时器、计数器、CTH 和 CTC 的编号。", Environment.NewLine, "3. 未登录监控器，却要进行监控器读取。" };
                return string.Concat(textArray1);
            }
        }

        public virtual string KeyenceNanoE1
        {
            get
            {
                string[] textArray1 = new string[] { "1. 发送了CPU单元不支持的指令。", Environment.NewLine, "2. 指定指令的方法出错。", Environment.NewLine, "3. 确立通讯前，发送了 CR 以外的指令。" };
                return string.Concat(textArray1);
            }
        }

        public virtual string KeyenceNanoE2 =>
            ("1. 在 CPU 单元没有存储程序的状态下， 发送了“M1（切换到 RUN 模式）”指令。" + Environment.NewLine + "2. 在 CPU 单元的 RUN/PROG 开关处于PROG 状态下，发送了“M1（切换到RUN 模式）”指令。");

        public virtual string KeyenceNanoE4 =>
            "想要更改写入去能程序的定时器、计数器和 CTC 的设定值。";

        public virtual string KeyenceNanoE5 =>
            "在尚未排除CPU单元错误的情况下， 发送了“M1( 切换到RUN模式)”指令。";

        public virtual string KeyenceNanoE6 =>
            "读取“RDC”指令选定的软元件中。";
        ...
        public virtual string KeyenceSR2000Error23 =>
            "由于正在连接 AutoID Netwoerk Navigator, 因此不能接收 RS-232C 发送的命令";

        public virtual string KeyenceSR2000Error99 =>
            "如果觉得SR-2000系列有异常，请联系基恩士公司";
        ...
        public virtual string MelsecError51 =>
            "在使用随机访问缓冲存储器的通讯时，由外部设备指定的起始地址设置在 0-6143 的范围之外。解决方法:检查及纠正指定的起始地址。";

        public virtual string MelsecError52 =>
            "1. 在使用随机访问缓冲存储器的通讯时，由外部设备指定的起始地址+数据字数的计数(读时取决于设置)超出了 0-6143 的范围。\r\n2. 指定字数计数(文本)的数据不能用一个帧发送。(数据长度数值和通讯的文本总数不在允许的范围之内。)";

        public virtual string MelsecError54 =>
            "当通过 GX Developer 在[操作设置]-[通讯数据代码]中选择“ASCII码通讯”时，则接收来自外部设备的、不能转换为二进制代码的ASCII 码。";

        public virtual string MelsecError55 =>
            "当不能通过 GX Developer(无检查标记)来设置[操作设置]-[无法在运行时间内写入]时，如 PLCCPU 处于运行状态，则外部设备请求写入数据。 ";

        public virtual string MelsecError56 =>
            "从外部进行的软元件指定不正确。";

        public virtual string MelsecError58 =>
            "1. 由外部设备指定的命令起始地址(起始软元件号和起始步号)可设置在指定范围外。\r\n2. 为扩展文件寄存器指定的块号不存在。\r\n3. 不能指定文件寄存器(R)。\r\n4. 为位软元件的命令指定字软元件。\r\n5. 位软元件的起始号由某一个数值指定，此数值不是字软元件命令中16 的倍数。";

        public virtual string MelsecError59 =>
            "不能指定扩展文件的寄存器。";

        public virtual string MelsecErrorC04D =>
            "在以太网模块通过自动开放 UDP端口通讯或无序固定缓冲存储器通讯接收的信息中，应用领域中指定的数据长度不正确。";

        public virtual string MelsecErrorC050 =>
            "当在以太网模块中进行 ASCII 代码通讯的操作设置时，接收不能转化为二进制代码的 ASCII 代码数据。";
        ...
        public virtual string SiemensDataLengthCheckFailed =>
            "数据块长度校验失败，请检查是否开启put/get以及关闭db块优化";
        ...
        public virtual string YokogawaLinkError03 =>
            "设备地址不存在，或以字为单位错误地指定了继电器进行读写访问。";

        public virtual string YokogawaLinkError04 =>
            "超出设置范围的值：1. 0和1以外的字符用于位设置。 2.字设置超出了有效范围0000至FFFF。 3.命令中指定的起始位置（例如“加载/保存”）超出有效地址范围。";

        public virtual string YokogawaLinkError05 =>
            "数据计数超出范围：1.指定的位数，字数等超出了规格范围。 2.指定的数据计数和设备参数计数等不匹配。";

        public virtual string YokogawaLinkError06 =>
            "尝试执行监视而未指定监视命令（BRS，WRS）";
        ...
        public virtual string YokogawaLinkError44 =>
            "接收字符时超时：1.未接收到结束字符或ETX。 2.超时时间为5秒";

        public virtual string YokogawaLinkError51 =>
            "超时错误：1.由于诸如CPU电源故障之类的原因，没有从CPU返回过程结束响应。（超时）2.顺序CPU硬件故障。 3.顺序CPU不接受命令。 4.顺序CPU服务时间不足";

        public virtual string YokogawaLinkError52 =>
            "CPU在处理期间检测到错误。";

        public virtual string YokogawaLinkErrorF1 =>
            "内部错误：1.在执行Load（PLD）或Save（PSV）命令以外的命令期间，发出了Cancel（PLC）命令。 2.检测到内部错误。";
    }
}
